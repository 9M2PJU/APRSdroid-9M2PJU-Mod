...
object LocationSource {
	...
	def getPermissions(prefs : PrefsWrapper) = {
		prefs.getString("loc_source", DEFAULT_CONNTYPE) match {
			case "smartbeaconing" => Set(AndroidManifest.permission.ACCESS_FINE_LOCATION, AndroidManifest.permission.ACCESS_COARSE_LOCATION)
			case "periodic" => Set(AndroidManifest.permission.ACCESS_FINE_LOCATION, AndroidManifest.permission.ACCESS_COARSE_LOCATION)
			case "manual" => Set()
		}
	}
}
